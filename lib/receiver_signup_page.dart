...
class ReceiverSignUpPage extends StatefulWidget {
  const ReceiverSignUpPage({super.key});
...
class _ReceiverSignUpPageState extends State<ReceiverSignUpPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final exists = await AppDatabase.receiverExists(cnic.text);
      if (exists) {
        _showError("This CNIC is already registered as a receiver");
        return;
      }

      await AppDatabase.insertReceiver(
        name: name.text,
        phone: phone.text,
        cnic: cnic.text,
        city: city,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Receiver Registration")),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          const Text(
            "Join as a Receiver 🍽️",
